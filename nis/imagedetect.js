...
ImageData.fromUrl = async function (url, callback, callbackarg) {
	// i would have changed this to gamma cleared variant, but apparently dgkey depends on (bugged) gamma
	var img;
	img = new Image();
...
ImageData.fromUrlClearGamma = async function (url, callback, callbackarg) {
	var imgfile = new Uint8Array(await fetch(url).then(q => q.arrayBuffer()));
	ImageData.fixPngBuffer(imgfile);
	var base64 = "";
	for (var i = 0; i < imgfile.length; i++) {
		base64 += String.fromCharCode(imgfile[i]);
	}
	ImageData.fromBase64(callback, btoa(base64));
}
